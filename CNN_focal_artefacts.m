...
startTime = "23:10";   % find time from table
...
endTime = "02:59";   % find time from table
...
for file_number = 1:num_edf_files % iteratre through the subject's included files/recordings
    ...
    % Find focally marked artefacts
    prefix = char('/projects3/EPIHFO/SEEGOMICS/Pat76/76 Pat-EEG' + string(sscanf(file_name, '%*[^0-9]%d')));
    efile = dir([prefix '*.xlsx']);
