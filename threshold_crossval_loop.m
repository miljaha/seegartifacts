  ...
  % if testsubj == 27 || testsubj == 47; continue; end % skip example subjects
    for sn = 1:size(trainsubjects) % loop training subjects
        ...
        for file_number = 1:num_edf_files % iteratre through the subject's included files/recordings
            ...
            epilept_probs = [epilept_probs, pat_probs];
            clear data;
        end
        ...
        epileptic_prob = mean(epilept_probs,1);
        epileptic_prob = epileptic_prob(:,startblock:min(endblock,size(epilept_probs,2))); 
        % crop artefacts
        ...
        epil_chan_prob = mean(epilept_probs,2);
