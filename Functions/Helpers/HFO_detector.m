%% Detection of High-Frequency Oscillations in SEEG Data
% Mohammad Al-Sa'd, 2025
...
% Notes: The general band-pass filter order is increased from 200 to 400.
% Also, the filter error weights are now symmetric ([10 1 1] to [10 1 10]).
%
function [FastRipples, Events] = HFO_detector(Data, fs)
%% Parameters
...
Freq_band_range = [250 285 330 380 435 500];    % Passbands of the narrowband fitlers
Amp_freq_edge   = [0 0 1 1 0 0];                % Amplitude for each frequency edge
Error_weight    = [10 1 1];                     % Error weight vector in dB
...
for k = 1:length(Freq_band_range)-1 % iterate across frequency bands
    ...
    Events{k} = [DetectionChannel Central_freq(k)*ones(NumberOfEvents,1),EventStart EventLength];
...
FastRipples = cell(1,num_Channels);
for i = 1:num_Channels % iterature through channels
    ...
    while size(EventsInChannel,1) > 0
        ...
        % events are represented as [channel, mean central frequency, start, duration]
    ...
    FastRipples{i} = ChannelEvent(1:Counter,:); % Collect results from each channel
end
FastRipples = cat(1,FastRipples{:}); % Concatenate all characteristics
