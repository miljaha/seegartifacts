...
%   -bw length of tested segaxent on beta-activity in second
...
for i=1:length(index_stop)
    ...
    if useMatObj
        ...
        thisstart = index_start(i);
        thisstop = min(index_stop(i),size(d,1));
        [sub_d,sub_envelope,sub_background,sub_discharges,sub_out,sub_envelope_pdf,RFactor]=spike_detector(d(thisstart:thisstop,:),fs,winsize,noverlap);
    end
