...
%
%
% Coded May [U+200E]27, [U+200E]2022, by Darion Toutant,
...
if ~isempty(AnnoIDX)
    ...
    %% MA updated
    first_anno_bytes = Anno(1:AnnoIDX(1));                       % raw bytes of first annotation
    cleaned_str = char(first_anno_bytes(first_anno_bytes > 0));  % remove NULLs and convert to char
    tokens = regexp(cleaned_str', '\+?[-]?\d+(\.\d+)?', 'match'); % extract numeric part
    if ~isempty(tokens)
        EDF_Start = str2double(tokens{1});                       % convert to double
    else
        warning('Failed to parse EDF start annotation. Defaulting to 0.');
        EDF_Start = 0;
    end
    %%
    ...
    EDFhdr.Annotations(1).value      = [];
    EDFhdr.Annotations(1).sample     = [];
    EDFhdr.Annotations(1).timestamp  = [];
    ...
    for x = 1:numel(AnnoIDX) - 1
        ...
        if TALS21idx
            ...
            EDFhdr.Annotations(count).timestamp = str2double(CurrentAnno(1:TALS21idx(1)-1)) - EDF_Start;
            EDFhdr.Annotations(count).sample = round(1 + EDFhdr.SamplingRate(1).*EDFhdr.Annotations(count).timestamp);
            EDFhdr.Annotations(count).value = CurrentAnno(TALS20idx(1)+1:TALS20idx(end)-1);
            count = count + 1;
        ...
        elseif numel(TALS20idx) >= 2
            ...
            for xx = 1:numel(TALS20idx) - 1
                ...
                if diff(TALS20idx) ~= 1
                    ...
                    EDFhdr.Annotations(count).timestamp = str2double(CurrentAnno(1:TALS20idx(1)-1)) - EDF_Start;
                    EDFhdr.Annotations(count).sample = round(1 + EDFhdr.SamplingRate(1).*EDFhdr.Annotations(count).timestamp);
                    EDFhdr.Annotations(count).value = CurrentAnno(TALS20idx(xx)+1:TALS20idx(xx+1)-1);
                    count = count + 1;
                end
            end
        end
