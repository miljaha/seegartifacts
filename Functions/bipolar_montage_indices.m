...
for k = 1:Nch-1
    ...
    if strcmpi(txt1,txt2) && str2double(num1)==str2double(num2)-1  % Same electrode and adjacent contact?                  
        %bipo_labels=vertcat(bipo_labels,['EEG ' tmp1 '-' tmp2]);  % New bipo label          
...
[npairs,~]=size(bipo_inds);
disp(['Total ' num2str(npairs) ' SEEG contact pairs detected']);
disp(['Total ' num2str(length(el_labels)) ' SEEG electrodes detected']);
%disp(el_labels);

disp('--- Bipolar montage for SEEG-data created ---');
