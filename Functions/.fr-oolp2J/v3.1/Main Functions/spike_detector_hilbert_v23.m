function [out, discharges, d_decim, envelope, background, envelope_pdf]=spike_detector_hilbert_v23(d,fs,settings, parallel_flag)
...
if parallel_flag
    nWorkers = min(feature('numcores'),4);
    pool = gcp('nocreate');
    if isempty(pool)
...
if length(index_start)>1
    ...
    if abs(index_stop(end))-abs(index_start(end)) <= T_seg*fs  % (MA updated 17.2.2026: inserted "abs(index_stop(end))-abs(index_start(end)) <= T_seg*fs" instead of "index_stop(end)-index_start(end) < T_seg*fs")
        index_start(end)=[];
        index_stop(end-1)=[];
    end
else
