...
TP_train = segment_extra(:,:,1:n_train_extra);
TP_test = segment_extra(:,:,n_train_extra+1:n_train_extra+n_test_extra);
TP_val = segment_extra(:,:,n_train_extra+n_test_extra+1:end);

% combine to X and y
% ADD EXTRA SAMPLES HERE!!!
X_train = cat(3,TN_train, TP_train );
y_train = [(repmat([2],1,n_train_TN)),(repmat([1],1,n_train_TP)),3];
y_train = categorical(y_train, [1 2 3], {'noise','ok','patology'});
y_train = y_train(1:end-1);

X_test= cat(3,TN_test, TP_test);
y_test= [(repmat([2],1,n_test_TN)),(repmat([1],1,n_test_TP)),3];
y_test = categorical(y_test, [1 2 3], {'noise','ok','patology'});
y_test = y_test(1:end-1);

X_val = cat(3,TN_val, TP_val);
y_val = [(repmat([2],1,n_test_TN)),(repmat([1],1,n_test_TP)),3];
